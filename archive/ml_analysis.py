import pandas as pd
import numpy as np
from sklearn.linear_model import LogisticRegression
from sklearn.tree import DecisionTreeClassifier, export_text
from sklearn.model_selection import cross_val_score
from sklearn.preprocessing import StandardScaler
from src.database import AuctionDatabase

# Connect to database
db = AuctionDatabase()

# Pull enriched data
print("Fetching data...")
data = db.query("""
    SELECT
        fiscal_year,
        fiscal_week_number,
        avg_lot_value,
        goal_status,
        construction_pct,
        construction_alv,
        ag_pct,
        ag_alv,
        passenger_pct,
        passenger_alv,
        trucks_med_heavy_pct,
        trucks_med_heavy_alv,
        pct_items_10k_plus,
        total_items_sold
    FROM weekly_metrics_summary_enrichedv2
    WHERE has_auctions = true
""")

db.close()

# Convert to DataFrame
df = pd.DataFrame(data)
print(f"Loaded {len(df)} weeks of data")

# Create target variable (1 = hit, 0 = miss)
df['target'] = (df['goal_status'] == 'hit').astype(int)

# Fill NaN ALV values with 0 (weeks where that industry had no items)
alv_cols = ['construction_alv', 'ag_alv', 'passenger_alv', 'trucks_med_heavy_alv']
df[alv_cols] = df[alv_cols].fillna(0)

# Features to use
features = [
    'construction_pct', 'construction_alv',
    'ag_pct', 'ag_alv',
    'passenger_pct', 'passenger_alv',
    'trucks_med_heavy_pct', 'trucks_med_heavy_alv',
    'pct_items_10k_plus', 'total_items_sold'
]

X = df[features]
y = df['target']

print(f"\nTarget distribution: {y.sum()} hits, {len(y) - y.sum()} misses")

# ============================================================
# LOGISTIC REGRESSION
# ============================================================
print("\n" + "="*60)
print("LOGISTIC REGRESSION")
print("="*60)

# Scale features for logistic regression
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)

lr = LogisticRegression(max_iter=1000, random_state=42)
lr.fit(X_scaled, y)

# Cross-validation accuracy
lr_cv_scores = cross_val_score(lr, X_scaled, y, cv=5)
print(f"\nCross-validation accuracy: {lr_cv_scores.mean():.1%} (+/- {lr_cv_scores.std()*2:.1%})")

# Feature importance (coefficients)
print("\nFeature Importance (coefficient magnitude):")
coef_df = pd.DataFrame({
    'feature': features,
    'coefficient': lr.coef_[0],
    'abs_coef': np.abs(lr.coef_[0])
}).sort_values('abs_coef', ascending=False)

for _, row in coef_df.iterrows():
    direction = "+" if row['coefficient'] > 0 else "-"
    print(f"  {direction} {row['feature']}: {row['coefficient']:.3f}")

# ============================================================
# DECISION TREE
# ============================================================
print("\n" + "="*60)
print("DECISION TREE")
print("="*60)

# Try different max_depth to avoid overfitting
for depth in [2, 3, 4]:
    dt = DecisionTreeClassifier(max_depth=depth, random_state=42)
    dt_cv_scores = cross_val_score(dt, X, y, cv=5)
    print(f"\nDepth {depth} - CV accuracy: {dt_cv_scores.mean():.1%} (+/- {dt_cv_scores.std()*2:.1%})")

# Use depth=3 for interpretability
dt = DecisionTreeClassifier(max_depth=3, random_state=42)
dt.fit(X, y)

print("\n" + "-"*40)
print("Decision Tree Rules (depth=3):")
print("-"*40)
print(export_text(dt, feature_names=features))

# Feature importance from tree
print("\nFeature Importance (from tree):")
importance_df = pd.DataFrame({
    'feature': features,
    'importance': dt.feature_importances_
}).sort_values('importance', ascending=False)

for _, row in importance_df.iterrows():
    if row['importance'] > 0.01:
        print(f"  {row['feature']}: {row['importance']:.1%}")

# ============================================================
# COMPARE TO MANUAL SCORECARD
# ============================================================
print("\n" + "="*60)
print("COMPARISON TO MANUAL SCORECARD")
print("="*60)

# Manual scorecard prediction
df['scorecard_pred'] = ((df['construction_pct'] >= 25) & (df['construction_alv'] >= 16000)).astype(int)
scorecard_acc = (df['scorecard_pred'] == df['target']).mean()

# ML predictions
df['lr_pred'] = lr.predict(X_scaled)
df['dt_pred'] = dt.predict(X)

lr_acc = (df['lr_pred'] == df['target']).mean()
dt_acc = (df['dt_pred'] == df['target']).mean()

print(f"\nManual Scorecard (Const>=25%, ALV>=$16k): {scorecard_acc:.1%}")
print(f"Logistic Regression: {lr_acc:.1%}")
print(f"Decision Tree (depth=3): {dt_acc:.1%}")

# Confusion matrices
print("\n--- Manual Scorecard ---")
print(f"True Positives:  {((df['scorecard_pred']==1) & (df['target']==1)).sum()}")
print(f"False Positives: {((df['scorecard_pred']==1) & (df['target']==0)).sum()}")
print(f"False Negatives: {((df['scorecard_pred']==0) & (df['target']==1)).sum()}")
print(f"True Negatives:  {((df['scorecard_pred']==0) & (df['target']==0)).sum()}")

print("\n--- Decision Tree ---")
print(f"True Positives:  {((df['dt_pred']==1) & (df['target']==1)).sum()}")
print(f"False Positives: {((df['dt_pred']==1) & (df['target']==0)).sum()}")
print(f"False Negatives: {((df['dt_pred']==0) & (df['target']==1)).sum()}")
print(f"True Negatives:  {((df['dt_pred']==0) & (df['target']==0)).sum()}")

# ============================================================
# WHAT THE ML FOUND
# ============================================================
print("\n" + "="*60)
print("KEY INSIGHTS FROM ML")
print("="*60)
print("""
The decision tree will show you the ACTUAL thresholds that best
split the data, and which features matter most. Compare these
to your manual scorecard to see if you're using the right
thresholds and the right features.
""")
